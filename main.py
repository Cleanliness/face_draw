...
def train():
    gen = cnn.ConvGenerator(image_size, 20).to(device)
    ...
    torch.save(discr.state_dict(), "discr.pt")
    return gen, discr


def train_patchgan():
    gen = cnn.ConvGenerator(image_size, 20).to(device)
    discr = patch_gan.PatchGAN().to(device)
    ...
    while curr_it < iterations:
        for i, batch in enumerate(dataloader, 0):
            ...
            # compute discriminator CE, backprop
            optimizer_D.zero_grad()
            lbl_l_shape = (batch_size//2,) + discr_out_real.size()[1:]
            lbl_r_shape = (math.ceil(batch_size/2),) + discr_out_real.size()[1:]

            lbl_left = torch.zeros(lbl_l_shape).to(device)
            lbl_discr = torch.ones(lbl_r_shape).to(device)
            lbl_discr = torch.cat((lbl_left, lbl_discr))
            discr_out_real = torch.cat((discr_out_gen, discr_out_real))

            discr_cost = loss_f(discr_out_real, lbl_discr) / 2
            discr_cost.backward()
            print(float(discr_cost))
            optimizer_D.step()
            ...
            f_lbl_shape = (batch_size//2,) + discr_out_real.size()[1:]
            fake_lbl = torch.ones(f_lbl_shape).to(device)
            gen_cost = loss_f(discr_out_fake_gen, fake_lbl)
            gen_cost += lam*l1(gen_out_old, f[:batch_size//2])
